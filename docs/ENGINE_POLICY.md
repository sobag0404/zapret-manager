...
The manager starts verified Flowseal `general*.bat` strategies with Windows
ShellExecute `runas`. The GUI itself does not require administrator rights; the
engine launch requests UAC only when the user presses `Включить`. Current
selectable strategies are `general`, `alt`, `alt2`, `alt3`, `simple_fake`, and
`fake_tls_auto`. Free-form argument entry is a developer feature and must never
be enabled for normal users without clear risk labeling.
